// 日付と時刻のフォーマット部品。
var formatter = {}
formatter.formatDateSelf = function(date, pattern) {
  if (date == null) {
    return ''
  }
...
formatter.formatDateSelf.defaultFormat = 'yyyy-MM-dd HH:mm:ss'
formatter.formatMoney = function(source, digit) {
  if (source == null) {
    return ''
  }
  ...
  var numberR = source
      ...
      .split('')
      .reverse(),
    digitR = source.split('.')[1]
  var result = ''
  for (i = 0; i < numberR.length; i++) {
    result += numberR[i] + ((i + 1) % 3 == 0 && i + 1 != numberR.length ? ',' : '')
  }
...
formatter.getFormattedDate = function() {
  return formatter.formatDateSelf(new Date(), 'yyyy-MM-dd')
}

formatter.getFormattedDateTime = function() {
  return formatter.formatDateSelf(new Date(), 'yyyy-MM-dd HH:mm')
}
